...
- ✅ **Demo `arena-pelucias` (tema futebol/masculino) substituiu Alice na landing operador (04/05/2026):** Ivo achou Alice "muito feminina" pra apresentar a operadores (publico majoritariamente masculino). Criado tenant `arena-pelucias` (id 6) + site default (id 5) — "Arena Pelucias", **dourado #c9a227** (visual tropheu/premium), brand emoji ⚽, brand tag "mascotes oficiais · todo time, todo torcedor", hero "O mascote do seu time, premio garantido em cada jogada", emojis ⚽🏆🦁🐯🦅 (mascotes diversos), badges Box premiada/Mascotes oficiais/Cashback 7%, R$ 8,00. Link demo na landing operador trocado: `?tenant=alice-pelucias` → `?tenant=arena-pelucias` (3 ocorrencias). Alice mantida no banco (id 4) como tenant teste mas sem link publico. SW v9 → v10.
- ✅ **Landing operador refinada com mini-mapa + 3 modos de pagamento (04/05/2026):** apos feedback do Ivo (mais didatica, sem jargao tecnico). Adicionado **mini-mapa interativo** no inicio da landing (`#op-map-container`, Leaflet, 5 pins demo BR com status colorido, popup ao clicar, sem zoom/drag — visual estatico mas vivo). Nova secao **3 modos de pagamento** logo apos o mapa: (1) "No local · sem cadastro" PIX/cartao maquininha, (2) "No site · sem cadastro" QR + PIX/cartao no celular, (3) "No site · cliente cadastrado" VIP — mensagens das suas promocoes, bonus, pacotes, assinatura mensal, cashback (com `pay-mode-3` em destaque rainbow). **ESP32 → "modulo de telemetria"** em todas as referencias publicas (jargao tecnico tirado). **Prazos especificos removidos** ("24 horas pra cadastro", "10 min pra instalar" → linguagem vaga "rapido", "plug-and-play"). FAQ "Quanto tempo pra estar no ar" reescrita sem promessa de horas. SW v8 → v9.
- ✅ **Fase 1.1 — Whitelabel + landing operador profissional (04/05/2026):** ja em prod com primeiro tenant demo `alice-pelucias` cadastrado pra demonstrar. **Backend** novo modulo `platform_public.py` com `GET /v1/public/tenants/{tenant_key}/site-config` (publico, sem auth, retorna `name`, `logo_url`, `primary_color`, `texts` do `Site.texts_json` do tenant). **Front** `loadSiteConfig()` detecta tenant via (1) `?tenant=<slug>` query, (2) subdominio `<slug>.clubedasgruas.com.br`, (3) default `pelucias-demo`. `applySiteConfig()` substitui `--pink` CSS variable, brand emoji, brand text, brand tag, hero eyebrow, hero h1 (com `<span class="grad">`), hero paragraph, hero badges (4 dinamicos), hero emojis (5 ursinhos), footer tagline. `DEFAULT_TEXTS` evita layout quebrado em fallback. Mesma URL → tenant diferente = site visualmente diferente (cor, logo, copy, badges, emojis, preco). Validado: `?tenant=alice-pelucias` muda 100% do visual (lavanda #9061d6, 💖, "As pelucias mais fofas...", cashback 8%, R$ 7,00). **Landing operador** refeita pra apresentar projeto a interessados: hero com pitch claro, secao "Antes vs Depois" (problema/solucao em 2 colunas), 4 passos pra entrar, 6 diferenciais (whitelabel, clientes compartilhados, promocoes proprias, financeiro completo, comandos remotos, galeria como marketing), card "Veja como fica" com link demo `?tenant=alice-pelucias`, FAQ com 6 perguntas honestas, CTA WhatsApp. SW v7 → v8. Smoke prod: HTTPS clubedasgruas 200, IP:8081 200, IP:8081?tenant=alice-pelucias 200, autopasso 200 (intacto).
...
- **04/05/2026** - Claude Code - **Demo arena-pelucias (futebol) substituiu Alice na landing operador.** Tenant `arena-pelucias` (id 6) + site default (id 5) criados via API com tema masculino: dourado #c9a227, ⚽, mascotes oficiais (emojis ⚽🏆🦁🐯🦅), copy "O mascote do seu time, premio garantido em cada jogada", R$ 8,00, cashback 7%. Link demo na landing trocado em 3 lugares (`?tenant=alice-pelucias` → `?tenant=arena-pelucias`). Alice mantida no banco como tenant teste mas sem link publico. SW v9 → v10. Smoke prod: HTTPS 200, autopasso 200 (intacto).
- **04/05/2026** - Claude Code - **Landing operador refinada (apos feedback Ivo).** Mini-mapa Leaflet interativo no inicio (5 pins demo BR, popups, sem zoom/drag — visual estatico). Nova secao **3 modos de pagamento** destacando (1) no local sem cadastro, (2) no site sem cadastro, (3) no site cadastrado VIP com mensagens/bonus/pacotes/assinatura mensal — modo VIP com gradient rainbow pra destacar a oportunidade. **ESP32 → "modulo de telemetria"** em todas as referencias publicas (sem jargao tecnico). **Prazos especificos removidos** (24h, 10min) e substituidos por linguagem flexivel ("rapido", "plug-and-play"). SW v8 → v9. Deploy: HTTPS 200, autopasso 200 (intacto).
- **04/05/2026** - Claude Code - **Fase 1.1: Whitelabel + landing operador profissional.** Backend novo `platform_public.py` com `GET /v1/public/tenants/{tenant_key}/site-config` (suite freepix 168/168 mantida). Tenant `alice-pelucias` (id 5) criado com config diferente (cor lavanda #9061d6, 💖, R$ 7,00, 8% cashback, copy proprio) pra demonstrar whitelabel. Front: `loadSiteConfig()` detecta tenant via querystring/subdominio/default, `applySiteConfig()` aplica cor, logo, brand, hero, badges, emojis, footer dinamicamente. Landing operador refeita pra apresentar projeto: problema/solucao 2 colunas (6×6 itens), 4 passos, 6 diferenciais (whitelabel/clientes compartilhados/promocoes proprias/financeiro/remotos/galeria), demo card linkando `?tenant=alice-pelucias`, FAQ com 6 Q&A, CTA WhatsApp. SW v7 → v8. Deploy ok: HTTPS 200, IP:8081 200, autopasso 200 (intacto). Cross-project commits: freepix `(novo)` + demo_pelucias `(novo)`.
